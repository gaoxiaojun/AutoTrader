...
// CRunningData5::CRunningData5(void)
// {
// 	//BOOLDAY = 20;
// 	//M3_Std = 3;
// }
...
void CRunningData5::_init_( vector<CMinuteData> inputData )
{
...
void CRunningData5::calculateIdx( void )
{
	//int BOLLDAY = 20;
	//int M3_Std = 3;
	...
	for (int i=0; i<roo; i++)
	{
		...
		{
			float testFloat = m_Data_Vec.at(k).GetClosePrice();
...
void CRunningData5::AddNewStrategyData( StrategyData inputData )
{
	// first check the new strategy data
	// wether in the head of m_data
}

void CRunningData5::RefreshIndex()
{
	calculateIdx();
}

void CRunningData5::PopData()
{
	m_Boll_mid.pop_back();
	m_Boll_up.pop_back();
